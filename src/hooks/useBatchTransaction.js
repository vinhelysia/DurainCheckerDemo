...
export function useBatchTransaction({
  ...
  const registerBatch = async (
    ...
    if (providerMode === 'chain' && program && wallet.publicKey) {
      try {
        ...
        const txSig = await program.methods.registerBatch(
          batchId,
          farmVi,
          provinceVi,
        ...
        // Add timeline event "Thu hoạch" automatically
        try {
          const freshBatch = await program.account.batch.fetch(batchPda)
          const freshCount = freshBatch.timelineCount

          const timelineEventPda = getTimelinePda(batchId, freshCount, program.programId)
          ...
          console.warn('Failed to register initial timeline event', evtErr)
        }
  ...
  const updateLabReport = async (
    ...
    if (providerMode === 'chain' && program && wallet.publicKey) {
      try {
        ...
        const txSig = await program.methods.updateLabReport(
          selectedBatchId,
          new BN(cadmiumValueScaled),
          new BN(thresholdValueScaled),
          new BN(confidenceScaled),
        ...
        try {
          ...
          const timelineEventPda = getTimelinePda(selectedBatchId, freshCount, program.programId)
          const logisticsRolePda = getLogisticsPda(wallet.publicKey, program.programId)
      ...
      try {
        ...
        // update overall risk profile to match latest report
  ...
  const addTimelineEvent = async (
    ...
    if (providerMode === 'chain' && program && wallet.publicKey) {
      try {
        ...
        setTxMessage({
          text: language === 'vi' ? `Thành công! Đã thêm chặng [${stageVi}] vào Blockchain. Mã Tx: ${txSig.slice(0, 16)}...` : `Success! Added stage [${stageVi}] to Blockchain. Tx Signature: ${txSig.slice(0, 16)}...`,
          type: 'success',
          txSig: txSig
        })
  ...
  const handleRoleAction = async (actionType, targetAddress, targetRole) => {
